...
// React Hook Form + shadcn/ui integration (manual — not in shadcn v4 registry)
...
function FormMessage({ className, children, ...props }: React.ComponentProps<"p">) {
  ...
  if (!body) return null;
  return <p data-slot="form-message" id={formMessageId} className={cn("text-destructive text-sm font-medium", className)} {...props}>{body}</p>;
}
